...
public class CreateServer implements Closeable {
   ...
   public static void main(final String[] args) {
      ...
      String sshPublicKeyFileName = System.getProperty("user.home") + "/.ssh/google_compute_engine.pub";
      String sshPublicKey = null;
      try {
         sshPublicKey = Files.toString(new File(sshPublicKeyFileName), Charset.defaultCharset());
      } catch (IOException e) {
         System.err.println("Cannot open your SSH public key at " + sshPublicKeyFileName
                + "\nIt is required to authorize your access to the machine.\n"
                + "See https://developers.google.com/compute/docs/instances#sshkeys for more details.\n"
                + e.getMessage());
         System.exit(1);
      }
      String sshPrivateKeyFileName = System.getProperty("user.home") + "/.ssh/google_compute_engine";
      String sshPrivateKey = null;
      try {
         sshPrivateKey = Files.toString(new File(sshPrivateKeyFileName), Charset.defaultCharset());
      } catch (IOException e) {
         System.err.println("Cannot open your SSH private key at " + sshPrivateKeyFileName
